...
如果某个文件块的哈希值在数据库中成功匹配，则表明该块已存在于服务器中，此时服务器仅需在块哈希值文件中记录一个引用，而无需传输实际数据块。相反，如果数据库中没有匹配的哈希值，则需要将该文件块传输到服务器，并在数据库中创建新记录，同时在块哈希值文件中为该块添加相应的引用。

对于一个文件，服务器上最终会有以下三种记录：

- **唯一的文件块**：这是源文件中的实际数据块。对于重复的数据块，服务器中只保留一个唯一副本。所有不重复的文件块以单个或成组的形式存储。在本系统中，这些文件称为 Unique Block Files（.ubk 文件）。
...
为了存储源文件中唯一的数据块，对于每一个源文件本系统将这些块组合成一个独立文件的形式进行保存，称为“唯一文件块”，文件格式为 `.ubk`（Unique Block）。每个文件块只保留一个副本，避免存储重复的数据块。在处理重复的文件块时，系统会通过引用机制确保同一块数据不会被重复存储。将多个数据块组合存储为一个整体文件形式，也能避免频繁打开和关闭文件流所带来的读写性能问题，从而提高系统的效率。
