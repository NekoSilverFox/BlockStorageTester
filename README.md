...
![data_stream.drawio](doc/img/data_stream.drawio.svg)

该系统的应用场景如下：用户计算机的本地存储中包含源文件。当用户通过互联网将文件传输到服务器时，文件会首先在用户本地逐块分割，并为每个块计算哈希值。此哈希值将与服务器数据库中的哈希记录进行比对。同时，服务器上还会为该源文件创建一个对应的块哈希值文件，以便后续的文件恢复。该文件会按顺序记录源文件中每个块的哈希值。

如果某个文件块的哈希值在数据库中成功匹配，则表明该块已存在于服务器中，此时服务器仅需在块哈希值文件中记录一个引用，而无需传输实际数据块。

相反，如果数据库中没有匹配的哈希值，则需要将该文件块传输到服务器，并在数据库中创建新记录，同时在块哈希值文件中为该块添加相应的引用。

对于一个文件，服务器上最终会有以下三种记录：

- **唯一的文件块**：存储源文件中的实际数据块。对于重复的数据块，服务器中只保留一个唯一副本。所有不重复的文件块以单个或成组的形式存储。在本系统中，这些文件称为 Unique Block Files（.ubk 文件）。
- **数据库**：存储文件块的哈希值、源块的位置等信息，用于后续查找和匹配。
- **块哈希值文件**：此文件包含源文件所有块对应的哈希值，用于源文件的恢复。在本系统中，该文件称为 Block Hash Files（.bkh 文件）。

通过以上方法，可以显著减轻云服务中的文件传输和存储压力。
...
根据“应用场景及系统分析”章节，我们确定了本项目中需要解决的三项**存储任务**：文件块信息在数据库中的记录、唯一文件块的存储以及块哈希值文件的存储。具体设计如下：
...
为了存储源文件中所有不重复的数据块，对于每一个源文件本系统将这些不重复得块组合成一个独立文件的形式进行保存，称为“唯一文件块”，文件格式为 `.ubk`（Unique Block）。块将不设置分割符的连续写入文件，后续借助 C/C++的指针进行定位和读取。

在处理源文件重复的文件块时，系统会通过引用机制确保同一块数据不会被重复存储。将多个块组合存储为一个整体文件形式，也能避免频繁打开和关闭文件流所带来的读写性能问题，从而提高系统的效率。

下图展示了块大小为 4Bytes ，连续无分隔符的唯一块文件。

![image-20241007054133021](doc/img/image-20241007054133021.png)

### 块哈希文件存储

为了记录源文件每个数据块的哈希值，我们采用独立文件进行存储，称之为“块哈希文件”，文件格式为 `.bkh`（Block Hash）。

由于不同哈希算法生成的哈希值长度是固定的，因此采用连续存储的方式，将每个块的哈希值顺序写入文件。在文件恢复过程中，通过指针移动固定的间隔即可读取对应的哈希记录，并通过数据库中记录的信息重建源文件。

下图展示了哈希算为 MD5 ，连续无分隔符的块哈希文件。

![image-20241007054754067](doc/img/image-20241007054754067.png)
...
![image-20241007165253809](doc/img/image-20241007165253809.png)
...
- **异步计算模块**：此模块负责执行源文件的分块和恢复任务，是核心的计算执行部分。异步计算模块能够并行处理任务，提升系统的响应性和性能。它与数据库模块交互，执行数据库的读写操作，同时调用哈希模块进行哈希计算。最终的计算结果通过信号传递给 GUI 模块进行展示。
...
下图中展示了数据库中块信息的记录和块哈希文件(.bkh)的内容。可以看到长度为 16 字节的哈希值既写入到了数据库中，又连续写入到了文件当中。

![image-20241007005004376](doc/img/image-20241007005004376.png)

下图展示了源文件及去除重复块后的唯一块文件（.ubk）。可以看到，源文件中重复的块已成功识别并未保存到唯一块文件中。

![image-20241007010117115](doc/img/image-20241007010117115.png)
...
![image-20241007165423125](doc/img/image-20241007165423125.png)
...
从分块时间来看，随着块大小的增大，分割用时呈指数级减少。例如，4KB 块的分块时间为 **3062.74 秒**，而 2048KB 块只需要 **5.132 秒**。这反映出小块虽然能有效地识别重复数据，但由于需要进行更多的哈希计算和数据库操作，每个块的处理时间也相对增加。相比之下，较大的块减少了块的数量，从而降低了系统的负载，提高了分块速度。

文件的恢复时间也表现出类似的趋势。当使用 4KB 块时，恢复时间为 **1269.83 秒**，而 2048KB 块的恢复时间仅为 **2.15 秒**。恢复操作的用时同样受块数量影响，较小的块需要进行更多的 I/O 操作和哈希匹配，因此恢复时间较长。而大块减少了操作次数，使得恢复过程更加高效。
